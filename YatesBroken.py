...
def deployYatesForward(treatment_results_in_Yates_order, all_cols=False):
    ...
    # cols is the exact same thing

    for col_num in range(num_columns):
        print(col_num)
        ctr = 0
        # print("len(cols[col_num]):", len(cols[col_num]))
        # Add phase
        while ctr < len(treatment_results_in_Yates_order):
            # print(cols[col_num][ctr + 1])
            print("len(treatment_results_in_Yates_order)", len(treatment_results_in_Yates_order))
            print("ctr:", ctr)
            print("cols[col_num][{}]:".format(ctr + 1), cols[col_num][ctr + 1])
